...
def select_date():
    ...
    while True:
        ...
        if vol_dt < start_date or vol_dt > end_date:
            print("\nPlease enter a date from " + start_date.strftime('%d-%m-%Y') + " to " + end_date.strftime(
                '%d-%m-%Y') + " (inclusive).")
...
def select_start_time(vol_date, cur_user_times):
    ...
    print("There must be at least 1 hour between volunteering sessions. "
          "Please cancel your existing sessions first if necessary.")
    ...
    if len(available) == 0:
        ...
        logging.warning("No available start times on the selected date. "
    ...
    while True:
        ...
        if start not in available:
            print("\nPlease enter an available start time in the format HH:mm.")
            logging.error("Invalid user input.")
            continue
...
def select_end_time(start_time, cur_user_times):
    ...
    while True:
        ...
        try:
            ...
            print("\nPlease enter a number corresponding to one of the available end times.")
            logging.error("Invalid user input.")
            continue
